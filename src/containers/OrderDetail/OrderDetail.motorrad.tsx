...
// TODO: AFTER GO LIVE FOR MOTORRAD
...
const stateToStepMap: Record<OrderState, number> = {
  ...
  [OrderState.Available]: 2,
  [OrderState.Confirmed]: 3,
  [OrderState.WaitingForCreditDecision]: 3,
...
const OrderDetail: FC<ServerSideData> = props => {
  ...
  const { setCurrentDealershipCode } = useDealershipContext()
  const {
    data,
    error: orderError,
    isLoading,
  ...
  if (order.state === OrderState.Available && beginOrderStep === BeginOrderSteps.BeginOrder) {
    return (
      <OrderLayout activeStep={activeStep} serverSideProps={props}>
        <BeginOrder
          order={order}
          vehicle={vehicle}
          customer={customer}
  ...
  //   ) {
  //     return (
  //       <OrderLayout activeStep={activeStep}>
