...
const messages = {
  title: 'Completed',
  description: 'Below is the list of completed orders.',
  loadMore: 'Load more',
}
...
const Appoinments: FC<{ dealerCode?: string }> = ({ dealerCode }) => {
  ...
  return showDetails ? (
    <>
      <SectionHeading icon="done" subtitle={showDetails && messages.description} className="mt-5" itemCount={total}>
        {messages.title}
      </SectionHeading>
      {showDetails && <CompletedTable data={pageData} isLoading={isLoading} onRowNavigate={viewOrder} />}
      {showDetails && (!isExhausted || isLoading) && (
        <Button
          tertiary
          onClick={loadMore}
          fullWidth
          className="mt-2 utm-workqueue-loadmore-btn-completed-orders"
          loading={isLoading}
        >
          {messages.loadMore}
        </Button>
